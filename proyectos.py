...
def proyectos_estrategicos():
    ...
    proyectos_tlrd = pd.read_csv(StringIO(s3_data))

    st.dataframe(proyectos_tlrd)
    st.subheader('Proyectos 2050')
    ...
    if st.button("Generar"):
        if user_promt:
            with st.spinner("Generando respuesta:"):
                    prompt_result = pandas_ai.run(data_frame=proyectos_tlrd, prompt=f'{user_promt}')
